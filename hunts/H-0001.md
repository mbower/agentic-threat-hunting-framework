...
Detect macOS information stealing malware (T1005) using AppleScript commands to duplicate sensitive user data files—including Safari cookies, Notes databases, and user documents—by identifying file duplication operations targeting browser data stores and user document directories for staging and exfiltration.

### Threat Context

Atomic Stealer and similar macOS information stealers employ AppleScript commands to systematically collect sensitive data from infected systems. This technique is classified as **T1005 - Data from Local System** and falls under the **Collection** tactic (TA0009).
...
| search (file_path="*/Library/Cookies/Cookies.binarycookies"
...
| search NOT (process_name IN ("Safari", "Google Chrome", "Firefox", "Notes", "securityd", "cloudd"))
...
```bash
# On suspected macOS system, check for staged files in common locations
find /tmp /var/folders ~/Library/Caches -name "Cookies.binarycookies" -o -name "NoteStore.sqlite*" -mtime -7 2>/dev/null

# Check for recently modified AppleScript files or compiled scripts
find /tmp ~/Downloads ~/Desktop -name "*.scpt" -o -name "*.app" -mtime -3 -ls

# Review process execution history (requires EDR or process accounting)
log show --predicate 'process == "osascript"' --last 7d | grep -i "duplicate\|cookies\|notestore"

# Check for suspicious LaunchAgents (persistence mechanism)
ls -la ~/Library/LaunchAgents/ /Library/LaunchAgents/ | grep -v "com.apple"

# Inspect network connections from suspicious processes (if process still running)
lsof -i -P | grep -i <suspicious_process_name>

# Check code signing of suspicious binary
codesign -dvvv /path/to/suspicious_binary
spctl --assess --verbose /path/to/suspicious_binary
```
...
This hunt investigated macOS data collection activity (T1005) targeting sensitive user files over a 7-day period. Analysis of 500,000 process execution events and 2 million file operations identified one confirmed instance of information stealing malware behavior on executive system MAC-EXE-042. The hypothesis was confirmed: an unsigned binary executed AppleScript commands to systematically duplicate Safari cookies, Notes database files, and filtered documents from Desktop/Documents directories to a staging location in /tmp. The malware attempted to exfiltrate collected data to an external IP address before being detected. This represents a successful data theft attempt caught during the collection phase, before full exfiltration occurred.
...
| search NOT process_name IN ("Safari", "Google Chrome", "Firefox", "Notes", "securityd", "cloudd", "mds", "mdworker")
...
| join type=left hostname process_name [
    ...
    | search dest_ip!=10.* dest_ip!=172.16.* dest_ip!=192.168.*
...
**Next Review:** 2025-12-19 (recurring monthly macOS threat hunt)
